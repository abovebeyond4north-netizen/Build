...
FROZEN_COMPONENTS = (
    "protected_evaluator",
    "hidden_world_generator_genesis16",
    "representation_genesis13",
    "partial_observation_interface_genesis15",
    "measurement_update_genesis15",
...
def assert_genesis16_architecture(
    ...
    if changed_frozen:
        raise ArchitectureViolation(
            "Genesis-16 changed frozen components: "
            + ", ".join(changed_frozen)
        )
    ...
    if not changed_mutable:
        raise ArchitectureViolation(
            "Genesis-16 changed none of its permitted experimental components"
        )
